...
void sort_gr_by_count(struct word_gr arr[]);
void write_groups(struct word_gr arr[]);
void write_groups_by_num(struct word_gr arr[]);
void write_groups_by_names(struct word_gr arr[]);
...
int main(int argc, char **argv)
{
  ...
  sort_gr_by_count(groups);
  write_groups_by_names(groups);
  return 0;
}
